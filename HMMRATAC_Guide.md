...
```java -jar HMMR_ATAC-2.0.0.jar -b <SortedBAM> -i <BAMIndex> -g <GenomeStatsFile> <options>```
...
* ```-i , --index <BAI>```
   ...
   This version supports using .csi index files.
...
* ```--window <int>```
   ...
   running HMMRATAC on A desktop or a machine with limited memory. Be aware
   that lowering this will affect output accuracy. Fixing this issue is
   a work in progress so check back at https://github.com/Mikxox/HMMRATAC
   for updates.
...
* ```--binary <True || False>```
   ...
   This version outputs both a binary model and a human_readable model.
   Using either model will give you the same output but the non-binary
   model allows users to manually change values. Default is False.
...
3. Name.model / Name_readable.model
  ...
  model after running HMMRATAC. If HMMRATAC
...
1. Java heap space error:
   ...
   The original implementation had some memory usage issues, but those
   should now be fixed. If you do have any problems try running using
   the -Xmx<integer>m option to increase allowed java memory amount.
   ```java -Xmx3072m HMMR_ATAC-2.0.0.jar -b ATACseq.sorted.bam -i ATACseq.sorted.bam.bai -g genome.info```
